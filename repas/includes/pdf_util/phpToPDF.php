...
function Header()
{
	if (isset($_SESSION['entete']) && $_SESSION['entete']=='OUI')
	{
		if (isset($_SESSION['etat']))
		{
			switch ($_SESSION['etat'])	
			{
				case 'GROUPE' : {	$titreEtat = 'LISTE DES GROUPES' ; break ; }
				case 'FICHE' : {	$titreEtat = 'FICHE ETUDIANT' ; break ; }
				case 'LISTE1' : {	$titreEtat = 'Liste de classe BTS1' ; break ; }
				case 'LISTE2' : {	$titreEtat = 'Liste de classe BTS2' ; break ; }
				case 'LISTE1SISR' : {	$titreEtat = 'Liste de classe BTS1 SISR' ; break ; }
				case 'LISTE1SLAM' : {	$titreEtat = 'Liste de classe BTS1 SLAM' ; break ; }
				case 'LISTE2SISR' : {	$titreEtat = 'Liste de classe BTS2 SISR' ; break ; }
				case 'LISTE2SLAM' : {	$titreEtat = 'Liste de classe BTS2 SLAM' ; break ; }
				default : {	$titreEtat = $_SESSION['etat'] ; break ; }
			}
		}
		
		//entête Bon ENLEVEMENT
		$this->SetXY(5,5);
		$this->Cell(5,5,'',0,0,'L','',5,5,50,18);
		$this->SetFont('Arial','',7);
		//adresse et téléphone
		if(isset($_SESSION['orientation']) && $_SESSION['orientation']=='P')
		{
			//$this->Ln(10);
			$this->SetFont('Arial','B',15);
			$this->SetXY(10,5);
			$this->Cell(282,5,$titreEtat, 0,0,'C') ;
		}
		else
		{
			$this->SetFont('Arial','B',15);
			$this->SetXY(10,5);
			$this->Cell(192,5,$_SESSION['titreEtat'], 0,0,'C') ;
		}
		$this->Ln(10);
	}
	else
	{
		...
		$this->Cell(5,6,'Le '.date('d/m/Y'), 0,0,'L') ;
		$this->SetFont('Arial','B',12);
		//Décalage
		$titre=$_SESSION['Titre_Etat_PDF'] ;
		$this->Cell(0,6,$titre,0,0,'C');
		$this->SetFont('Arial','I',8);
